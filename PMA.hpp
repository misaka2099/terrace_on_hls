...
// #endif
  ...
  PMA(PMA &other);
  ~PMA();
  void double_list(uint64_t task_id, custom_vector<uint_t> &sub_counts, uint_t num_elements);
  void half_list(uint64_t task_id, custom_vector<uint_t> &sub_counts, uint_t num_elements);
  //void half_list();
  ...
  void redistribute_par(uint_t index, uint64_t len, custom_vector<uint_t> &sub_counts, uint_t num_elements, bool for_double = false);
  ...
  uint64_t get_size();
  uint64_t get_n();
  custom_vector<std::tuple<uint32_t, uint32_t, uint32_t>> get_edges();
  void clear();
  // bool check_no_locks();

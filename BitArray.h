...
inline uint64_t bit_array_count(uint32_t *array, uint64_t length) {
  uint64_t count = 0;
...
inline bool is_not_empty(uint32_t *array, uint64_t length) {
  //uint64_t count = 0;
  for (uint64_t i = 0; i < length/32; i++) {
    if (array[i] > 0)
			return true;
...
class BitArray {
  ...
  BitArray(const BitArray &other) {
    length = other.length;
    array = (uint32_t *) mymalloc(length/8);
    to_free = true;

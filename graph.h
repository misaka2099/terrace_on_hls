...
static inline void unlock(uint32_t *data)
{
  ...
  class Graph {
    public:
      ...
      typedef custom_set<vertex> vertex_set;
      typedef custom_set<edge> edge_set;
      ...
      int add_edge(const vertex s, const vertex d);
			...
			void add_edge_batch(vertex *srcs,vertex *dests, weight *w, uint32_t
													edge_count, custom_vector<uint32_t>& perm);
#else
			void add_edge_batch(vertex *srcs,vertex *dests, uint32_t edge_count,
        custom_vector<uint32_t>& perm);
#endif
...
*/
  ...
  inline  Graph::Graph(uint32_t size) : second_level(size), num_vertices(size) {
    inplace_size = size * sizeof(vertex_block);
    vertices = (vertex_block*)mymalloc(size, sizeof(vertex_block));
  }
  ...
  inline bool Graph::check_in_place_dup(const vertex s, const vertex d) const {
...
inline void Graph::add_inplace(vertex *srcs, vertex	*dests, weight *wghts,
													...
													*array_btree_node) {
#else
inline void Graph::add_inplace(vertex *srcs, vertex *dests, uint32_t idx,
    custom_vector<uint32_t>& parts, uint8_t *array_pma,
													uint8_t *array_btree, uint8_t *array_btree_node) {
#endif
		...
		if (degree == 0) { // it's a new src. Copy neighbors to in place
			...
			uint32_t in_place_limit = degree < NUM_IN_PLACE_NEIGHBORS ? degree :
				NUM_IN_PLACE_NEIGHBORS;
			vertex *new_in_place = (vertex*)mymalloc(degree + size, sizeof(vertex));
#if WEIGHTED
			weight *new_in_place_wghts = (weight*)mymalloc(degree + size, sizeof(vertex));
#endif
...
#if WEIGHTED
inline void Graph::add_btree(vertex *srcs, vertex	*dests, const weight *wghts,
												uint32_t i, custom_vector<uint32_t>& parts, uint8_t
												*array) {
#else 
inline 	void Graph::add_btree(vertex *srcs, vertex	*dests, uint32_t i,
    custom_vector<uint32_t>& parts, uint8_t *array) {
#endif
...
// add edge in batch
#if WEIGHTED
inline 	void Graph::add_edge_batch(vertex *srcs, vertex *dests, weight *wghts,
														 uint32_t edge_count, std::vector<uint32_t>& perm)
	{
#else
inline void Graph::add_edge_batch(vertex *srcs, vertex *dests, uint32_t edge_count,
    custom_vector<uint32_t>& perm) {
#endif
		//BitArray array_pma(edge_count);
		//BitArray array_btree(edge_count);

		uint8_t *array_pma = (uint8_t*)mymalloc(edge_count, sizeof(uint8_t));
		uint8_t *array_btree = (uint8_t*)mymalloc(edge_count, sizeof(uint8_t));

		// generate partitions array
		custom_vector<uint32_t> parts;
		...
		uint8_t *array_btree_node = (uint8_t*)mymalloc(parts.size(), sizeof(uint8_t));
...
#if WEIGHTED
inline int Graph::add_edge(const vertex s, const vertex d, const weight w) {
#else
inline int Graph::add_edge(const vertex s, const vertex d) {
#endif
    uint32_t idx;
#if ENABLE_LOCK
    lock(&vertices[s].degree);
  ...
  inline uint32_t Graph::is_edge(const vertex s, const vertex d) {
	...
	inline int Graph::remove_edge(const vertex s, const vertex d) {
		uint32_t idx;
		int ret{0};
#if ENABLE_LOCK
		lock(&vertices[s].degree);
...
unlock:
  ...
  inline Graph::NeighborIterator Graph::neighbors(const vertex v) const {
  ...
  inline uint64_t Graph::get_size(void) {
  ...
  inline uint32_t Graph::get_num_edges(void) {
  ...
  inline uint32_t Graph::get_num_vertices(void) const {
    return num_vertices;
  }
...
#else
inline Graph::vertex Graph::NeighborIterator::operator*(void) const {
#endif
  ...
  inline void Graph::NeighborIterator::operator++(void) {
  ...
  inline bool Graph::NeighborIterator::done(void) const {
    if (local_idx == degree)
      return true;
  ...
  inline bool Graph::count_common_inplace(const vertex s, const vertex d, uint64_t&
                                   s_idx, uint64_t& d_idx, uint32_t& count)
    const {
  ...
  inline Graph::vertex Graph::get_pma_edge(uint64_t& idx) const {
  ...
  inline bool Graph::count_common_inplace_pma(const vertex s, const vertex d,
  ...
  inline bool Graph::count_common_inplace_btree(const vertex s, const vertex d,
                                         uint64_t& s_idx,
  ...
  inline bool Graph::count_common_pma_pma(const vertex s, const vertex d, uint64_t&
                                   s_idx, uint64_t& d_idx, uint32_t& count)
    const {
  ...
  inline bool Graph::count_common_pma_btree(const vertex s, const vertex d, uint64_t&
                                     s_idx, tl_container_iterator& d_it,
                                     uint32_t& count) const {
    uint64_t s_end = second_level.nodes[s].end;
  ...
  inline bool Graph::count_common_btree_btree(const vertex s, const vertex d,
                                       tl_container_iterator& s_it,
                                       tl_container_iterator& d_it,
                                       uint32_t& count) const {
#if WEIGHTED
  ...
  inline uint32_t Graph::count_common(const vertex s, const vertex d) const {
	...
	template<class F, typename VS> struct Btree_map {
		...
		void update(Graph::vertex v, Graph::weight w) {
			if (f.cond(v) == 1 && f.update(self_index, v, w) == 1) {
				if (output) {
					vs.insert_sparse(v);
				}
			}
		...
		void update(Graph::vertex v) {
			if (f.cond(v) == 1 && f.update(self_index, v) == 1) {
				if (output) {
					vs.insert_sparse(v);
				}
			}

...
class SlidingQueue {
 public:
  ...
  explicit SlidingQueue(const SlidingQueue &other, size_t shared_size) {
    ...
    for (uint64_t i = 0; i < shared_size; i++) {
      shared[i] = other.shared[i];
    }
  }

  ~SlidingQueue() {
    int x = shared_out_end - shared_out_start;
    mymuldel (shared,x);
  }
...
class QueueBuffer {
 public:
  ...
  explicit QueueBuffer(SlidingQueue<T> &master, size_t given_size = 16384)
    ...
    in = 0;
    local_queue = mymulnew (T,local_size);
  }

  ~QueueBuffer() {
    mymuldel( local_queue, local_size);
  }

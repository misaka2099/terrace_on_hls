...
void PMA::clear() {
  ...
  myfree((void*)edges.vals);
  myfree((void*)edges.dests);
...
void PMA::redistribute_par(uint_t index, uint64_t len, custom_vector<uint_t> &sub_counts, uint_t num_elements, bool for_double) {
  assert(find_leaf(&edges, index) == index);
  //printf("par len = %u\n", len);

  uint32_t *space_vals = (uint32_t *)mymalloc(64, len * sizeof(*(edges.vals)));
  uint32_t *space_dests = (uint32_t *)mymalloc(64, len * sizeof(*(edges.dests)));
  uint_t j = 0;
  ...
  */
  ...
  for (uint_t k = index; k < end; k+=REDISTRIBUTE_PAR_SIZE) {
...
void PMA::double_list(uint64_t task_id, custom_vector<uint_t> &sub_counts, uint_t num_elements) {
  ...
  */
  ...
  uint32_t *new_dests = (uint32_t *)mymalloc(32, new_N * sizeof(*(edges.dests)));
  uint32_t *new_vals = (uint32_t *)mymalloc(32, new_N * sizeof(*(edges.vals)));
  uint32_t * vals = (uint32_t *)edges.vals;
  uint32_t * dests = (uint32_t *)edges.dests;
  for (uint_t i = 0; i < new_N / 2; i++) {
    new_vals[i] = vals[i];
    new_dests[i] = dests[i];
  }
  for (uint_t i = new_N / 2; i < new_N; i++) {
...
void PMA::half_list(uint64_t task_id, custom_vector<uint_t> &sub_counts, uint_t num_elements) {
  //printf("doubling list by worker %lu\n", get_worker_num());
  // grab_all_locks(task_id, true, DOUBLE);
  uint64_t new_N = edges.N / 2;
  edges.loglogN = bsr_word(bsr_word(edges.N) - 1);
  ...
  */
  ...
  if (sub_counts.size() == 0) {
    ...
    for (size_t k = 0; k < sub_counts.size(); k+=1) {
      //printf("index = %u, len = %u, k = %u REDISTRIBUTE_PAR_SIZE = %u\n", index, len, k , REDISTRIBUTE_PAR_SIZE);
      uint_t start;
      if (k == 0) {
        start = 0;
...
void PMA::insert(uint64_t task_id, uint_t index, uint32_t elem_dest, uint32_t elem_val, uint32_t src
  ...
  for (uint32_t i = held_locks.x; i < locks_for_rebalance.x; i++) {
...
void PMA::remove(uint64_t task_id, uint_t index, uint32_t elem_dest, uint32_t src
  ...
  //printf("before with worker %lu\n", get_worker_num());
  //printf("worker %lu unlocking %d through %d with GENERAL \n", get_worker_num(), locks_for_leaf.x, locks_for_rebalance.x);
  for (int i = held_locks.x; i < locks_for_rebalance.x; i++) {
    nodes[i].lock.unlock(task_id);
  }
  //printf("worker %lu relasing locks %d through %d with reason REBALANCE\n",get_worker_num(), max(locks_for_rebalance.x, locks_for_leaf.x), locks_for_leaf.y);
...
void PMA::add_edge_batch_update_no_val_parallel(pair_uint *es, uint64_t edge_count) {
  ...
  myfree((void*)new_dests);
  myfree((void*)new_vals);

  uint32_t* final_vals = (uint32_t*) mymalloc(final_size * sizeof(uint32_t));
  uint32_t* final_dests = (uint32_t*) mymalloc(final_size * sizeof(uint32_t));
  ...
  myfree((void*)dense_dests);
  myfree((void*)dense_vals);
  //print_array(1);
  //check_sentinals();
  // release_all_locks(task_id, true, GENERAL);
  // node_lock.unlock_shared(0);
...
PMA::PMA(uint32_t init_n) {
  ...
  edges.vals = (uint32_t *)mymalloc(32, edges.N * sizeof(*(edges.vals)));
  edges.dests = (uint32_t *)mymalloc(32, edges.N * sizeof(*(edges.dests)));
  for (uint_t i = 0; i < edges.N; i++) {
    edges.vals[i] = 0;
